...
,
{ num: "2.", color: "text-blue-600", text: <>Match patient using the `identifiers` array (PhilHealth ID, MRN, etc.)</> },
{ num: "3.", color: "text-blue-600", text: "Format the data as a FHIR resource" },
{ num: "4.", color: "text-blue-600", text: <>POST the data to the `gatewayReturnUrl`</> },
]}
,
}}
...
{ num: "3.", color: "text-purple-600", text: <>Respond with `200 OK` to confirm receipt</> },
]}
`}
...
{ num: "3.", color: "text-amber-600", text: <>Respond with `200 OK` to confirm receipt</> },
]}
`}
...
Each identifier has a `system` (the namespace/authority)
and a `value` (the actual ID).
...
http://psa.gov.ph/birth-certificate
PSA Birth Certificate Number
1234-5678-9012-3456

http://your-hospital.com/mrn
Your Hospital's MRN
MRN-12345
...
**Matching Logic:** When you receive a query, try to match patients using ANY of the provided identifiers.
A patient may have multiple IDs - match on whichever one exists in your system.

### Initiate a Query

`{config.gatewayUrl}/api/v1/fhir/request/Patient`

**Response (202 Accepted)**

> **Push Data to Other Providers**
### Initiate a Push

`{config.gatewayUrl}/api/v1/fhir/push/Appointment`

**Response (200 OK)**

**Immediate Delivery:** Unlike queries, push requests are delivered immediately.
If the target provider accepts the data (returns 200 OK), the transaction is marked as COMPLETED instantly.

## Best Practices

:
practice.icon === "CheckCircle2" ?  :
...
Future handleProcessQuery(Request request) async {
try {
...
Future?> _findPatientByIdentifiers(List identifiers) async {
// Simulate DB lookup
...
Future handleReceiveResults(Request request) async {
try {
final payload = jsonDecode(await request.readAsString());
final String transactionId = payload['transactionId'];
final String status = payload['status'];
final Map? data = payload['data'];
