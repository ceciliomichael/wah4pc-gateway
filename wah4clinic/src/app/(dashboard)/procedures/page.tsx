...
export default function ProceduresPage() {
	...
	const fetchProcedures = async () => {
		try {
			...
			const bundle: ProceduresBundle = await response.json();
			setProcedures(bundle.entry?.map((entry) => entry.resource) || []);
		} catch (err) {
	...
	return (
		<div className="p-4 lg:p-8">
			<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
				<div>
					<h1 className="text-2xl sm:text-3xl font-semibold text-stone-900 mb-2">Procedures</h1>
					<p className="text-stone-600">
						{loading ? "Loading..." : `${procedures.length} procedure${procedures.length !== 1 ? "s" : ""} recorded`}
					</p>
				</div>
				<Link
					href="/procedures/new"
			...
			{loading ? (
				...
				<div className="bg-white rounded-2xl border border-stone-100 shadow-sm p-12">
					<div className="text-center">
						...
						<h3 className="text-lg font-medium text-stone-900 mb-2">No procedures recorded yet</h3>
						<p className="text-stone-600 mb-6">Get started by adding your first procedure</p>
						<Link
							href="/procedures/new"
				...
				<>
					...
					<div className="lg:hidden space-y-4">
						{procedures.map((procedure) => (
							<div key={procedure.id} className="bg-white rounded-2xl border border-stone-100 shadow-sm p-4 hover:shadow-md transition-shadow">
								<div className="flex items-start justify-between mb-3">
									<div className="flex items-center gap-2">
										<div className="w-10 h-10 bg-amber-100 rounded-full flex items-center justify-center flex-shrink-0">
											<LucideActivity className="w-5 h-5 text-amber-600" />
										</div>
										<div>
											<span className={`inline-block px-2 py-1 rounded-full text-xs font-medium ${getStatusBadgeColor(procedure.status)}`}>
												{procedure.status}
											</span>
										</div>
									</div>
								...
								<div className="space-y-2 mb-4">
									<div className="text-sm">
										<span className="text-stone-600 font-medium">Procedure:</span>
										<span className="text-stone-900 ml-2">{procedure.code?.text || "N/A"}</span>
									</div>
									<div className="text-sm">
										<span className="text-stone-600 font-medium">Patient:</span>
										<span className="text-stone-900 ml-2">{procedure.subject?.display || "N/A"}</span>
									</div>
									{procedure.category && (
										<div className="text-sm">
											<span className="text-stone-600 font-medium">Category:</span>
											<span className="text-stone-900 ml-2">{procedure.category.text}</span>
										</div>
									)}
									<div className="flex items-center gap-2 text-sm">
										<LucideCalendar className="w-4 h-4 text-stone-400" />
										<span className="text-stone-900">{formatDateTime(procedure.performedDateTime)}</span>
									</div>
								</div>
							...
							<table className="w-full">
								<thead className="bg-stone-50 border-b border-stone-200">
									<tr>
										...
										<th className="text-left px-6 py-4 text-sm font-medium text-stone-900">Procedure</th>
										<th className="text-left px-6 py-4 text-sm font-medium text-stone-900">Patient</th>
										<th className="text-left px-6 py-4 text-sm font-medium text-stone-900">Category</th>
										<th className="text-left px-6 py-4 text-sm font-medium text-stone-900">Performed</th>
										<th className="text-left px-6 py-4 text-sm font-medium text-stone-900">Actions</th>
									</tr>
								</thead>
								<tbody className="divide-y divide-stone-100">
									{procedures.map((procedure) => (
										<tr key={procedure.id} className="hover:bg-stone-50 transition-colors">
											<td className="px-6 py-4">
												<span className={`inline-block px-3 py-1 rounded-full text-xs font-medium ${getStatusBadgeColor(procedure.status)}`}>
													{procedure.status}
												</span>
											</td>
											<td className="px-6 py-4">
												<p className="font-medium text-stone-900">{procedure.code?.text || "N/A"}</p>
												<p className="text-sm text-stone-500">ID: {procedure.id.slice(0, 8)}</p>
											</td>
											<td className="px-6 py-4">
												<p className="text-stone-900">{procedure.subject?.display || "N/A"}</p>
											</td>
											<td className="px-6 py-4">
												<p className="text-stone-900">{procedure.category?.text || "N/A"}</p>
											</td>
											<td className="px-6 py-4">
												<div className="flex items-center gap-2 text-stone-900">
													<LucideCalendar className="w-4 h-4 text-stone-400" />
													<span className="text-sm">{formatDateTime(procedure.performedDateTime)}</span>
												</div>
											</td>
											<td className="px-6 py-4">
												<Link
