...
func TestGatewayServiceProcessResponse_WrapsSingleItemArrayAsBundle(t *testing.T) {
	t.Parallel()
	...
	case payload := <-requesterReceived:
		...
		if !ok || len(entry) != 1 {
			t.Fatalf("expected single bundle entry, got %#v", obj["entry"])
		}
...
func TestGatewayServiceProcessResponse_WrapsSingleObjectAsBundle(t *testing.T) {
	t.Parallel()
	...
	txRepo := newTxRepoStub()
	txRepo.items["txn-obj"] = model.Transaction{
		ID:           "txn-obj",
		RequesterID:  "requester",
		TargetID:     "target",
		ResourceType: "Observation",
...
func TestGatewayServiceProcessResponse_DoesNotBundleRejectedPayload(t *testing.T) {
	...
	txRepo := newTxRepoStub()
	txRepo.items["txn-rej"] = model.Transaction{
		ID:           "txn-rej",
		RequesterID:  "requester",
		TargetID:     "target",
		ResourceType: "Observation",
	...
	err := svc.ProcessResponse(
		IncomingResultPayload{
			...
			Data:          json.RawMessage(rejectedPayload),
		},
		"target",
		"Observation",
	)
